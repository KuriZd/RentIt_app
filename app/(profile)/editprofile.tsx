...
type SelectProps = {
  ...
  items: SelectItem[]; onChange?: (val: string) => void;
};
type FieldProps = {
...
function Select({ label, value, placeholder = "Select…", items, onChange }: SelectProps) {
  ...
  return (
    <View className="mb-4">
      {!!label && <Text className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">{label}</Text>}
      <Pressable
      ...
      >
        <Text className="text-base" style={{ color: value ? COLORS.text : COLORS.iconMuted }}>
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        ...
        <View className="absolute left-4 right-4 top-1/4 rounded-2xl p-3" style={[{ backgroundColor: COLORS.card }, shadow]}>
          <View className="mb-2 flex-row items-center justify-between">
            <Text className="text-base font-semibold" style={{ color: COLORS.text }}>{label || "Select"}</Text>
          ...
          <FlatList
            data={items} keyExtractor={(it, idx) => String(it?.value ?? idx)}
            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  <Text className="text-base" style={{ color: selected ? "#2563eb" : COLORS.text, fontWeight: selected ? "600" : "400" }}>
                    {item.label}
                  </Text>
                </Pressable>
              );
...
export default function ProfileEditScreen() {
  ...
  const years = useMemo<SelectItem[]>(() => {
    ...
    for (let y = now.getFullYear(); y >= now.getFullYear() - 100; y--) arr.push({ label: String(y), value: String(y) });
    return arr;
  }, []);
  const months = useMemo<SelectItem[]>(
    () => ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].map((m,i)=>({label:m,value:String(i+1).padStart(2,"0")})),
    []
  );
    ...
    async function load() {
      try {
        ...
        const { data, error } = await supabase
          .from("perfiles")
          .select("nombre, curp, estado_civil, email, fecha_nacimiento, direccion, avatar_url")
          .eq("id", user.id)
          .maybeSingle();
        ...
        if (data) {
          ...
          if (bd && /^\d{4}-\d{2}-\d{2}$/.test(bd)) {
            ...
            setDYear(y); setDMonth(m); setDDay(d);
          }
  ...
  const pickImage = async () => {
    ...
    if (status !== "granted") { Alert.alert("Permiso requerido", "Concede permiso para acceder a tus fotos."); return; }
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 0.9, allowsMultipleSelection: false, selectionLimit: 1,
    });
  ...
  const birthdate = dYear && dMonth && dDay ? `${dYear}-${dMonth}-${dDay}` : null;
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-white dark:bg-black">
        <ActivityIndicator size="large" color={scheme === "dark" ? "#E5E7EB" : "#000"} />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-black">
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingHorizontal: 16, paddingVertical: 16, paddingBottom: 140 }}>
        {/* Header */}
        <View className="mb-4 flex-row items-center">
          <Pressable onPress={() => router.back()} className="p-2 mr-2 rounded-full active:bg-zinc-100 dark:active:bg-zinc-800" accessibilityLabel="Volver">
            <Feather name="arrow-left" size={24} color={COLORS.text} />
          </Pressable>
          <Text className="text-3xl font-bold tracking-tight" style={{ color: COLORS.text }}>My Information</Text>
        </View>
        ...
        <View className="items-center mb-6">
          <Pressable onPress={pickImage} className="relative">
            <Image
              source={{ uri: avatarUri || "https://images.unsplash.com/photo-1560972550-aba3456b5564?q=80&w=1200&auto=format&fit=crop&ixlib=rb-4.1.0" }}
        ...
        <View className="mb-4 flex-row items-center gap-3">
          <View className="flex-1"><Select value={dDay} items={days} onChange={setDDay} placeholder="DD" /></View>
          <View className="flex-[1.2]"><Select value={dMonth} items={months} onChange={setDMonth} placeholder="MM" /></View>
          <View className="flex-[1.1]"><Select value={dYear} items={years} onChange={setDYear} placeholder="YYYY" /></View>
        </View>
        ...
        <Field label="Town (Municipio)" placeholder="Morelia" value={town} onChangeText={setTown} />
        <Field label="State (Estado)" placeholder="Michoacán" value={township} onChangeText={setTownship} />
        ...
        {/* Guardar */}
        <Pressable className="mt-2 h-12 items-center justify-center rounded-xl active:opacity-90"
          onPress={onSave} disabled={saving} style={{ backgroundColor: "#1677C3", opacity: saving ? 0.7 : 1 }}>
          <Text className="text-base font-semibold text-white">{saving ? "GUARDANDO..." : "SAVE"}</Text>
        </Pressable>
      </ScrollView>
