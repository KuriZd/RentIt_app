  ...
  const handlePickMedia = async () => {
    try {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        mediaTypes: ImagePicker.MediaTypeOptions.Images, // solo imágenes por ahora
        allowsEditing: true,
        quality: 0.8,
      });
      ...
      if (!reservation) {
        Alert.alert(
          "Espera un momento",
          "Todavía estamos cargando la información de la reserva."
        );
        return;
      }

      // 1) Pasar la URI a ArrayBuffer (compatible con RN)
      const response = await fetch(asset.uri);
      const arrayBuffer = await response.arrayBuffer();

      // 2) Extensión y content-type
      const ext =
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("reseñas").insert({
        ...
        url_media: mediaUrl, // aquí guardamos la URL pública
      });
          ...
          <View className="w-full max-w-xl self-center">
            ...
            >
              ...
              <Text
                ...
                style={{ color: COLORS.subtext }}
              >
                Tu foto puede ayudar a otros usuarios a conocer mejor el
                producto.
              </Text>
            </Pressable>
